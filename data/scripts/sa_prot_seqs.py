...
sa_fasta_path = "data/atlas_sa_sequences.fasta"

with open(sa_fasta_path, "w") as out:
    with open(fasta_path, "r") as fasta:
        # Load FASTA file using Biopython
        for name in SeqIO.parse(fasta, "fasta"):
            name_f = name.id.replace(".","_")
            out.write(">" + name.id + "\n")
            out.write(get_struc_seq("foldseek/bin/foldseek", 'data/atlas/analyses/{}/{}.pdb'.format(name_f,name_f))[''][-1]+"\n")
