...
import copy
import re
from transformers import T5Config, T5PreTrainedModel, T5EncoderModel, T5Tokenizer, AutoModel, AutoTokenizer, EsmConfig, EsmModel, EsmPreTrainedModel
...
def ESM2_classification_model(half_precision, class_config):
    ...
    if not half_precision:
        print("Full")
        model = AutoModel.from_pretrained("facebook/esm2_t33_650M_UR50D", local_files_only=True).to(torch.device('cuda:0'))
        tokenizer = AutoTokenizer.from_pretrained('facebook/esm2_t33_650M_UR50D', local_files_only=True)
    elif half_precision: 
        print("Half")
        tokenizer = AutoTokenizer.from_pretrained('facebook/esm2_t33_650M_UR50D', do_lower_case=False, local_files_only=True)
        model = AutoModel.from_pretrained("facebook/esm2_t33_650M_UR50D", torch_dtype=torch.float16, local_files_only=True).to(torch.device('cuda:0'))
    else:
        raise ValueError('Half precision can be run on GPU only.')
