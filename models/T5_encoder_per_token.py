...
def ESM2_classification_model(half_precision, class_config):
    if not half_precision:
        model = AutoModel.from_pretrained("facebook/esm2_t33_650M_UR50D", local_files_only=True)
        ...
        model = AutoModel.from_pretrained("facebook/esm2_t33_650M_UR50D", torch_dtype=torch.float16, local_files_only=True).to(torch.device('cuda'))
    else:
        raise ValueError('Half precision can be run on GPU only.')
